...
class _PenjualanTelurScreenState extends State<PenjualanTelurScreen> {
  ...
  Future<void> _loadData() async {
    ...
    final dataString = prefs.getString('penjualan_telur');
    if (dataString != null) {
      final List<dynamic> jsonData = jsonDecode(dataString);
      setState(() {
        _dataPenjualan = jsonData.cast<Map<String, dynamic>>();
      });
    }
  }
  ...
  Future<void> _saveData() async {
    final prefs = await SharedPreferences.getInstance();
    final dataString = jsonEncode(_dataPenjualan);
    await prefs.setString('penjualan_telur', dataString);
  }
            ...
            ElevatedButton.icon(
              onPressed: () async {
                final result = await Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => const TambahPenjualanTelurScreen()),
                );
                if (result != null && result is Map<String, dynamic>) {
                  setState(() {
                    _dataPenjualan.insert(0, result);
                  });
                  await _saveData(); // ✅ INI YANG PALING PENTING!
                }
              },
